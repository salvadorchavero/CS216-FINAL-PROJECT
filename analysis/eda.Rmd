...
```{r}
full_svm_cert <- svm(CERT_BIN ~ DECISION_DATE + JOB_INFO_EDUCATION +
                       WAGE_OFFER_FROM_9089 +
                       EMPLOYER_NUM_EMPLOYEES +
                       EMPLOYER_YR_ESTAB
                       , data = perm_svm)
...
# idk what this does exactly bc i copied it from somewhere but it looks useful
pred <- predict(full_svm_cert, perm_svm)
plot(pred)
```